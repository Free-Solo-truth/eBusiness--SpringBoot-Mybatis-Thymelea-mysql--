...
public class GoodsServiceImpl implements GoodsService{
	...
	public String addGoods(Goods goods, HttpServletRequest  request, String act) throws IllegalStateException, IOException {
		...
		MultipartFile AdvertiseFilename = goods.getAdvertiseFilename();
		...
		if(!myfile.isEmpty()) {
			...
			//获得上传文件原名
			//上传文件路径（开发环境）
			String path = "E:\\源代码\\eBusiness\\src\\main\\resources\\static\\images";
			//获得上传文件原名
		...
		if(!AdvertiseFilename.isEmpty()) {
			...
			String path = "E:\\源代码\\eBusiness\\src\\main\\resources\\static\\images";
			//获得上传文件原名
			String advertisefileName = AdvertiseFilename.getOriginalFilename();
			//对文件重命名
			String advertisefileNewName = MyUtil.getNewFileName(advertisefileName);
			File filePath = new File(path + File.separator + advertisefileNewName);
			//如果文件目录不存在，创建目录
			if(!filePath.getParentFile().exists()) {
				filePath.getParentFile().mkdirs();
			}
			//将上传文件保存到一个目标文件中
			AdvertiseFilename.transferTo(filePath);
			//将重命名后的图片名存到goods对象中，添加时使用
			goods.setAdvisepicture(advertisefileNewName);
		}
		if("add".equals(act)) {
